...
WebRtcDemo.App = (function (viewModel, connectionManager) {
    var _mediaStream,
        ...
        _recordStream = function (stream) {
            _mediaRecorder = new MediaRecorder(stream);
            _mediaRecorder.mimeType = 'video/mp4';
            _mediaRecorder.ondataavailable = function (blob) {
                console.log("Media Stream available", blob);
                //// POST/PUT "Blob" using FormData/XHR2
                var blobURL = URL.createObjectURL(blob.data);
                console.log("Blob Url", blobURL);
                //document.write('<a href="' + blobURL + '">' + blobURL + '</a>');
                var lsUrl = document.querySelector('.url.lastStream');
                lsUrl.href = blobURL;
                //lsUrl.click();
                //setTimeout(function () {
